...
class AngsuranPinjamanExport implements FromView, WithStyles
{
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->mergeCells('A1:F1'); // Sesuaikan sampai kolom J
        ...
        $borderRange = 'A3:F' . $lastRow; // Sesuaikan sampai kolom J
        ...
        $sheet->getStyle('A3:F3')->applyFromArray([ // Sesuaikan sampai kolom J
            'font' => ['bold' => true],
            'fill' => [
        ...
        for ($row = 4; $row <= $lastRow; $row++) {
            $sheet->getStyle("A{$row}:F{$row}")->applyFromArray([ // Sesuaikan sampai kolom J
                'borders' => [
                    'bottom' => [
                        'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
                    ],
