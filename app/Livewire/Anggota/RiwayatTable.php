...
class RiwayatTable extends Component
{
    ...
    public function render()
    {
        ...
        $riwayatGabungan = $kasHarian
            ->concat($qurbanRiwayat)
            ->filter(fn($item) => empty($item->skip_render))
            ->sortByDesc(function ($item) {
                $tanggal = \Carbon\Carbon::parse($item->tanggal)->format('Y-m-d H:i:s');
                $created = $item->created_at ? $item->created_at->format('Y-m-d H:i:s') : $tanggal;
                return $tanggal . $created;
            })
            ->values(); // reset index sebelum slice

        $page = request()->get('page', 1); // Ambil dari query string
        $perPage = 10;
        $total = $riwayatGabungan->count();
        $slice = $riwayatGabungan->slice(($page - 1) * $perPage, $perPage);
