<div>
  <form id="angsuranForm" action="{{ route('admin.angsuran.update', $angsuran->id) }}" method="POST">
      @csrf
      @method('PUT')
      <div class="mb-3">
      ...
      <div class="flex justify-start">
        <button data-modal-target="modal-pengurus-angsuran-pinjaman" data-modal-toggle="modal-pengurus-angsuran-pinjaman" onclick="initTomSelectPengurusBayarAngsuranPinjaman()" class="bg-green-800 text-white py-2 px-4 rounded-md" @if ($disabled) disabled @endif>
            Simpan
        </button>

        <div id="modal-pengurus-angsuran-pinjaman" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-full min-h-screen bg-gray-900 bg-opacity-40">
          <div class="relative p-4 w-full max-w-md max-h-full">
              ...
              <div class="relative bg-white rounded-lg shadow-xl">
                  ...
                  <div class="flex items-center justify-between p-5 border-b border-gray-200">
                      ...
                      <button type="button" class="text-gray-400 hover:bg-gray-100 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="modal-pengurus-angsuran-pinjaman">
                  ...
                  <div class="p-6 space-y-4">
                      <div>
                          ...
                          <select  id="select_pengurus_bayar_angsuran_pinjaman" name="created_by" class="text-left bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 transition duration-200" required>
                  ...
                  <div class="flex items-center justify-end p-6 pt-0 space-x-3 border-t border-gray-200 rounded-b">
                      <div class="mt-3">
                          <button data-modal-hide="modal-pengurus-angsuran-pinjaman" type="button" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white hover:bg-gray-100 border border-gray-300 rounded-lg focus:outline-none transition duration-200">
                              Batal
                          </button>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
      ...
      flatpickr("#datepicker-bayar-pinjaman", {
          dateFormat: "d-m-Y",
          allowInput: true,
          position: "below",
          locale: "id"
...
@push('scripts')
  <script>
      function initTomSelectPengurusBayarAngsuranPinjaman() {
          setTimeout(() => {
              if (!document.querySelector('#select_pengurus_bayar_angsuran_pinjaman').tomselect) {
                  new TomSelect("#select_pengurus_bayar_angsuran_pinjaman", {
                      create: false,
