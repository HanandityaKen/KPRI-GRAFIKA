<div>
    <form action="{{ route('admin.kas-harian.store') }}" id="formCreateJkm" method="POST">
        ...
        <div class="flex justify-start">
            <button data-modal-target="modal-pengurus-kas-masuk" data-modal-toggle="modal-pengurus-kas-masuk" onclick="initTomSelectPengurusCreateKasMasuk()" class="bg-green-800 text-white py-2 px-4 rounded-md" @if($disabled) disabled @endif>
                Simpan
            </button>
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                ...
                                <select wire:model.live="select_pengurus" id="select_pengurus_kas_masuk" name="created_by" class="text-left bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 transition duration-200" required>
...
@push('scripts')
    <script>
        function initTomSelectPengurusCreateKasMasuk() {
            setTimeout(() => {
                if (!document.querySelector('#select_pengurus_kas_masuk').tomselect) {
