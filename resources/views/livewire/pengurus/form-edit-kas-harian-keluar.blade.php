    ...
    <form action="{{ route('pengurus.kas-harian.update', $kasHarian->id) }}" id="formCreateJkk" method="POST">
        ...
        @if ($bendahara)       
            <div class="mb-4">
                ...
                <input wire:model.live="b_umum" type="text" id="b_umum" name="b_umum" class="format-rupiah bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2" placeholder="Masukan Nominal Biaya Umum" inputmode="numeric" value="{{ old('b_umum', $kasHarian->b_umum) }}" />
            </div>
            <div class="mb-4">
                <label class="block mb-1 text-sm font-medium text-gray-900">Biaya Organisasi</label>
                <input wire:model.live="b_orgns" type="text" id="b_orgns" name="b_orgns" class="format-rupiah bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2" placeholder="Masukan Nominal Biaya Organisasi" inputmode="numeric" value="{{ old('b_orgns', $kasHarian->b_orgns) }}" />
            </div>
            <div class="mb-4">
                <label class="block mb-1 text-sm font-medium text-gray-900">Biaya Operasional</label>
                <input wire:model.live="b_oprs" type="text" id="b_oprs" name="b_oprs" class="format-rupiah bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2" placeholder="Masukan Nominal Biaya Operasional" inputmode="numeric" value="{{ old('b_oprs', $kasHarian->b_oprs) }}" />
            </div>
            <div class="mb-4">
                <label class="block mb-1 text-sm font-medium text-gray-900">Biaya Lain</label>
                <input wire:model.live="b_lain" type="text" id="b_lain" name="b_lain" class="format-rupiah bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2" placeholder="Masukan Nominal Biaya Lain" inputmode="numeric" value="{{ old('b_lain', $kasHarian->b_lain) }}" />
            </div>
            <div class="mb-4">
