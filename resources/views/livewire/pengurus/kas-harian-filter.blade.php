    ...
    <div class="mb-8 flex justify-between items-center">
        ...
        @if ($bendahara)
            <a href="{{ route('pengurus.kas-harian.create') }}" wire:ignore class="bg-green-800 text-white py-2 px-4 rounded-md flex items-center ml-4">
                ...
                <span class="hidden sm:inline">Tambah Kas Harian</span>
            </a>
        @endif
    </div>
        ...
        <table class="w-full mb-8">
            <thead>
                <tr>
                    ...
                    <th class="p-3 text-center whitespace-nowrap border-l border-b border-[#6DA854]">Keterangan</th>
                    @if ($bendahara || $ketua)
            ...
            <tbody>
                @forelse ($kasHarians as $index => $kasHarian)
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        ...
                        <td class="p-3 text-left border-l border-[#6DA854]">{{$kasHarian->keterangan}}</td>
                        @if ($bendahara || $ketua)              
